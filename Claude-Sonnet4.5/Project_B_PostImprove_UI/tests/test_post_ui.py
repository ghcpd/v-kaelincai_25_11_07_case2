#!/usr/bin/env python3
"""
Test harness for Project B (Post-Improvement UI)
Tests the wizard with lazy loading and optimized field ordering
"""
...
class PostImprovementUITest:
    def __init__(self, base_url="http://localhost:8002", output_dir="screenshots", log_dir="logs", results_dir="results"):
        ...
        self.test_results = []
        self.log_file = self.log_dir / "log_post.txt"
    ...
    def run_test_case(self, browser, test_case, config):
        ...
        try:
            ...
            screenshot_path = self.output_dir / f"screenshots_post_{test_id}_initial.png"
            ...
            # IMPROVEMENT: Modal opens immediately with Step 1 visible (no loading needed)
            # Title field should be visible and focused
            page.wait_for_selector('#taskTitle', state='visible', timeout=5000)
            
            modal_open_end = time.time()
            modal_load_time_ms = (modal_open_end - modal_open_start) * 1000
            
            self.log(f"Modal opened in {modal_load_time_ms:.2f}ms (FAST - no lazy loading yet)")
            result['metrics']['modal_load_time_ms'] = modal_load_time_ms
            
            # Take modal screenshot - Step 1
            screenshot_path = self.output_dir / f"screenshots_post_{test_id}_step1.png"
            page.screenshot(path=screenshot_path)
            result['screenshots'].append(str(screenshot_path))
            
            # Check if title field is visible without scrolling (should be at top)
            ...
            if title_visible:
                ...
                self.log(f"Title field visible without scroll: {title_in_viewport} (IMPROVED)")
            else:
            ...
            result['metrics']['title_auto_focused'] = (focused_element == 'taskTitle')
            self.log(f"Title field auto-focused: {result['metrics']['title_auto_focused']}")
            
            # Get initial performance metrics
            browser_metrics = page.evaluate("() => window.performanceMetrics")
            if browser_metrics:
                self.log(f"Initial browser metrics: {json.dumps(browser_metrics, indent=2)}")
            ...
            if test_id == "TC004_malformed_input":
                ...
                self.complete_wizard(page, test_case['input'], result)
                ...
                try:
                    ...
                    screenshot_path = self.output_dir / f"screenshots_post_{test_id}_success.png"
                    page.screenshot(path=screenshot_path)
                    result['screenshots'].append(str(screenshot_path))
            ...
            # Screenshot on error
            try:
                screenshot_path = self.output_dir / f"screenshots_post_{test_id}_error.png"
                page.screenshot(path=screenshot_path)
                result['screenshots'].append(str(screenshot_path))
            except:
                pass
    ...
    def complete_wizard(self, page, input_data, result):
        """Complete the wizard flow step by step"""
        
        # STEP 1: Task Details (already visible, no loading)
        self.log("Step 1: Filling task details...")
        ...
        screenshot_path = self.output_dir / f"screenshots_post_{result['test_id']}_step1_filled.png"
        page.screenshot(path=screenshot_path)
        result['screenshots'].append(str(screenshot_path))
        
        # Click Next
        self.log("Navigating to Step 2...")
        step2_start = time.time()
        page.click('#nextBtn')
        
        # Wait for Step 2 to be visible
        page.wait_for_selector('.wizard-step[data-step="2"]', state='visible', timeout=5000)
        
        # LAZY LOADING: Wait for members and tags to load
        page.wait_for_selector('#assignee', state='visible', timeout=10000)
        page.wait_for_selector('#tags', state='visible', timeout=10000)
        
        step2_end = time.time()
        step2_load_time = (step2_end - step2_start) * 1000
        result['metrics']['step2_lazy_load_time_ms'] = step2_load_time
        self.log(f"Step 2 loaded in {step2_load_time:.2f}ms (includes lazy loading)")
        
        # Screenshot Step 2
        screenshot_path = self.output_dir / f"screenshots_post_{result['test_id']}_step2.png"
        page.screenshot(path=screenshot_path)
        result['screenshots'].append(str(screenshot_path))
        
        # STEP 2: Assignment & Tags
        self.log("Step 2: Selecting assignee and tags...")
        ...
        self.log("Navigating to Step 3...")
        step3_start = time.time()
        page.click('#nextBtn')
        
        # Wait for Step 3
        page.wait_for_selector('.wizard-step[data-step="3"]', state='visible', timeout=5000)
        
        # LAZY LOADING: Wait for attachments section
        page.wait_for_selector('#attachmentsList', state='visible', timeout=10000)
        
        step3_end = time.time()
        step3_load_time = (step3_end - step3_start) * 1000
        result['metrics']['step3_lazy_load_time_ms'] = step3_load_time
        self.log(f"Step 3 loaded in {step3_load_time:.2f}ms (includes lazy loading)")
        
        # Screenshot Step 3 with summary
        screenshot_path = self.output_dir / f"screenshots_post_{result['test_id']}_step3_summary.png"
        page.screenshot(path=screenshot_path)
        result['screenshots'].append(str(screenshot_path))
        
        # STEP 3: Review and Submit
        self.log("Step 3: Reviewing summary and submitting...")
        
        # Verify summary is displayed
        summary = page.locator('#taskSummary')
        summary_visible = summary.is_visible()
        result['metrics']['summary_displayed'] = summary_visible
        self.log(f"Summary displayed: {summary_visible}")
        
        # Submit
        self.log("Submitting task...")
    ...
    def run_validation_test(self, page, test_case, result):
        ...
        # Try to fill Step 1 with invalid data
        page.fill('#taskTitle', input_data.get('task_title', ''))
        page.fill('#dueDate', input_data.get('due_date', ''))
        ...
        step1_still_visible = page.locator('.wizard-step[data-step="1"]').is_visible()
        ...
        result['metrics']['validation_prevents_navigation'] = step1_still_visible
        
        self.log(f"Validation errors shown: {result['metrics']['validation_errors_shown']}")
        self.log(f"Navigation prevented: {result['metrics']['validation_prevents_navigation']}")
        
        # Screenshot validation errors
        screenshot_path = self.output_dir / f"screenshots_post_{test_case['test_id']}_validation.png"
        page.screenshot(path=screenshot_path)
        result['screenshots'].append(str(screenshot_path))
    
    def run_accessibility_test(self, page, test_case, result):
        """Test accessibility features in wizard"""
        # Check ARIA attributes
        modal = page.locator('#taskModal')
        aria_modal = modal.get_attribute('aria-modal')
        result['metrics']['aria_modal_present'] = aria_modal == 'true'
        
        # Check wizard progress has navigation role
        progress = page.locator('.wizard-progress')
        progress_role = progress.get_attribute('role')
        result['metrics']['wizard_progress_accessible'] = progress_role == 'navigation'
        
        # Fill form via keyboard
        page.fill('#taskTitle', test_case['input']['task_title'])
        page.fill('#dueDate', test_case['input']['due_date'])
        
        # Navigate via keyboard
        page.keyboard.press('Tab')
        page.keyboard.press('Enter')  # Click Next
        
        # Wait for Step 2
        time.sleep(0.5)
        
        # Complete via keyboard
        page.keyboard.press('Tab')  # Focus assignee
        page.keyboard.press('ArrowDown')  # Select option
        page.keyboard.press('Tab')  # Next field
        page.keyboard.press('Enter')  # Next step
        ...
        try:
            ...
            result['metrics']['keyboard_navigation_complete'] = True
            result['metrics']['success'] = True
        except:
            result['metrics']['keyboard_navigation_complete'] = False
            result['metrics']['success'] = False
    ...
    def check_pass_criteria(self, result, test_case):
        ...
        for key, expected in criteria.items():
            ...
            if actual != expected and expected is not False:
    ...
    def run_all_tests(self):
        """Run all test cases"""
        self.log("="*60)
        self.log("PROJECT B - POST-IMPROVEMENT UI TESTS")
        self.log("="*60)
    ...
    def save_results(self):
        ...
        with open(results_file, 'w', encoding='utf-8') as f:
            json.dump({
                'project': 'Project B - Post-Improvement',
                'timestamp': datetime.now().isoformat(),
